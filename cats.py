...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    new = [paragraph for paragraph in paragraphs if select(paragraph)]
    if k < len(new):
        return new[k]
    return ''
...
def about(keywords: list[str]):
    ...
    def checker(paragraph):
        temp = split(remove_punctuation(lower(paragraph)))
        for t in temp:
            for k in keywords:
                if t == k:
                    return True
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    if typed_word in word_list:
        ...
        for i in range(1, len(word_list)):
            if lowest > diff_function(typed_word, word_list[i], limit):
                lowest = diff_function(typed_word, word_list[i],limit)
                index = i
    if limit < lowest:
        return typed_word
    else:
...
def furry_fixes(typed: str, source: str, limit: int) -> int:
    ...
    """
    # BEGIN PROBLEM 6
    # base check is if they are eqaul, then return 0
    # want to compare if the first character is not equal, the turn first character into that character, then run the code again, with one less of the limit
    # if the limit is -1, this means that the limit has been reached, return the limit plus 1
    # when the word is longer, subtract typed word length from source length, and return the number, and same thing if it shorter
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    output = [[] for player in range(len(times))]

    for word in word_indices:
        fastest_player = 0
        fastest_time = get_time(times, 0, word)
        for x in player_indices:
            time = get_time(times, x, word)
            if time < fastest_time:
                fastest_time = time
                fastest_player = x
        output[fastest_player].append(words[word])
    return output
